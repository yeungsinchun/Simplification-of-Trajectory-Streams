...
class Line {
  public:
    ...
    Point start_point() { return u; }
    Point end_point() { return v; }
    double get_y(double x) { return -(A * x + C) / B; }
  ...
  public:
    ...
    void fit(const Point &rhs, double bound) {
        ...
        if (l.length() - this->length() < bound / 4) {
            ...
            this->theta +=
                f(l) * asin(this->calculate_distance(rhs) / (j * bound * 2)) /
                j;
            x = s.x + this->len * cos(this->theta);
            y = s.y + this->len * sin(this->theta);
            update_line_parmeters();
    ...
    double length() { return this->len; }

    double angle() { return this->theta; }

    bool is_first() { return j == 0; }
