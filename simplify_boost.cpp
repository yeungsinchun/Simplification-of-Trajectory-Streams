...
static BoostPolygon to_boost_polygon(const Polygon& poly) {
    BoostPolygon result;
    for (auto it = poly.vertices_begin(); it != poly.vertices_end(); ++it) {
        bg::append(result, BoostPoint(CGAL::to_double(it->x()), CGAL::to_double(it->y())));
    }
    bg::append(result, BoostPoint(CGAL::to_double(poly.vertices_begin()->x()), CGAL::to_double(poly.vertices_begin()->y())));
    return result;
}

static Polygon from_boost_polygon(const BoostPolygon& bp) {
    std::vector<Point> pts;
    for (auto it = bp.outer().begin(); it != bp.outer().end() - 1; ++it) {
        pts.emplace_back(bg::get<0>(*it), bg::get<1>(*it));
    }
    return Polygon(pts.begin(), pts.end());
}

static void intersect_boost(const Polygon& A, const Polygon& B,
                           std::back_insert_iterator<std::vector<Polygon_with_holes>> result) {
    BoostPolygon bA = to_boost_polygon(A);
    BoostPolygon bB = to_boost_polygon(B);
    std::vector<BoostPolygon> out;
    bg::intersection(bA, bB, out);
    for (const auto& bp : out) {
        Polygon p = from_boost_polygon(bp);
        if (p.is_simple() && !p.is_clockwise_oriented()) {
            *result++ = Polygon_with_holes(p);
        }
    }
}
...
// Output collectors for F and G polygons
std::vector<std::vector<Point>> all_F_polygons;
std::vector<std::vector<Point>> all_G_polygons;
...
static void print_help() {
    std::cout << "Usage: simplify [options]\n"
              ...
              << "  --out            Write output to data/taxi_simplified/<id>/original.txt & simplify_old.txt (resolved absolutely; requires --in <id>)\n"
...
int get_longest_stab(const std::vector<Point> &stream, int cur,
    ...
    while (cur < int(stream.size())) {
        ...
        for (int i = 0; i < int(P.size()); i++) {
            ...
            // Collect F and G for output files
            all_F_polygons.push_back(F);
            all_G_polygons.push_back(Gi);
            ...
            intersect_boost(F_poly, Gi_poly, back_inserter(new_S[i]));
...
int main(int argc, char** argv) {
    ...
    if (out_flag) {
        if (test_case_no == -1) {
            ...
            // 1. simplify_old.txt
            std::ofstream simp(dir / "simplify_old.txt");
            ...
            // 2. <eps>_<delta>_simplify_old.txt
            std::string param_name = std::to_string(EPSILON) + "_" + std::to_string(DELTA) + "_simplify_old.txt";
            ...
            // 3. old_F.txt - all F polygons
            std::ofstream oldF(dir / "old_F.txt");
            oldF << all_F_polygons.size() << '\n';
            for (const auto& poly : all_F_polygons) {
                oldF << poly.size() << '\n';
                for (const auto& p : poly) {
                    oldF << CGAL::to_double(p.x()) << ' ' << CGAL::to_double(p.y()) << '\n';
                }
            }
            oldF.close();

            // 4. old_G.txt - all G polygons
            std::ofstream oldG(dir / "old_G.txt");
            oldG << all_G_polygons.size() << '\n';
            for (const auto& poly : all_G_polygons) {
                oldG << poly.size() << '\n';
                for (const auto& p : poly) {
                    oldG << CGAL::to_double(p.x()) << ' ' << CGAL::to_double(p.y()) << '\n';
                }
            }
            oldG.close();
        }
    ...
    if (dist_flag && test_case_no != -1) {
        std::filesystem::path frechet_path = repo_root / "frechet_jl";
        // Quote the path to handle spaces (e.g., "Mobile Documents") and pass explicit args
        std::string cmd1 = std::string("\"") + frechet_path.string() + "\" --in " + std::to_string(test_case_no) + " --path \"" + (repo_root / "data" / "taxi_simplified" / std::to_string(test_case_no) / "simplify_old.txt").string() + "\"";
