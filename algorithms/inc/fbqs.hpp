...
class FBQS : public Algorithm {
        ...
        if (traj->size() == 1) {
            ...
            while (end < traj->size()) {
                ...
                if ((*traj)[start].distance((*traj)[end]) < bound) {
                    ...
                    auto d =
                        get_bounds(buffer_start, buffer_end, start, end, traj);
                    //         std::cout << "bound get " << d.first <<" " <<
                    //         d.second<< std::endl;
                    if (d.second < bound) {
  ...
  private:
    ...
    std::pair<double, double> get_bounds(int buffer_start, int buffer_end,
        ...
        //   std::cout << "Buffer size:" << buffer_end - buffer_start + 1 <<
        //   std::endl;
        int pos = 0;
        for (; i < buffer_end; i++) {
            //     std::cout << "start loop : " << i-1 << std::endl;
            Point buf_pt = (*traj)[i - 1];
            //      std::cout << buf_pt.x << " " << buf_pt.y << " " << st.x  <<
            //      " "  << st.y << std::endl;
        ...
        //      std::cout << "Pos" << pos << " " << quarter[pos][0] <<
        //      std::endl;
        ...
        for (; i < end_loop; i++) {
            ...
            if (quarter[i][0] == 0) {
                ...
                for (; k <= quarter[i][0]; k++) {
                    ...
                    if (first[pos]) {
                        min_ang_line =
                            Line{(*traj)[start], (*traj)[quarter[i][k]]};
                        max_ang_line =
                            Line{(*traj)[start], (*traj)[quarter[i][k]]};
                        first[pos] = false;
                    } else {
                ...
                Point c[4]{{min_x, max_y},
                           {max_x, max_y},
                           {max_x, min_y},
                           {min_x, min_y}};

                double d_corner_max = -1,
                       d_corner_min = std::numeric_limits<double>::max();

                for (int j = 0; j < 4; j++) {
                    d_corner_max =
                        std::max(d_corner_max, l.calculate_distance(c[j]));
                    d_corner_min =
                        std::min(d_corner_min, l.calculate_distance(c[j]));
                }
                Point l1 = min_ang_line.end_point(),
                      u1 = max_ang_line.start_point();
                Point l2, u2;
                // depends on corrdinate
                switch (i) {
                case 0:
                    l2 = Point{min_ang_line.get_x(c[3].y), c[3].y};
                    break;
                case 1:
                    l2 = Point{c[2].x, min_ang_line.get_y(c[2].x)};
                    break;
                case 2:
                    l2 = Point{min_ang_line.get_x(c[1].x), c[1].y};
                    break;
                case 3:
                    l2 = Point{c[3].x, min_ang_line.get_y(c[3].x)};
                    break;
                }

                switch (i) {
                case 0:
                    u2 = Point{c[3].x, max_ang_line.get_y(c[3].x)};
                    break;
                case 1:
                    u2 = Point{max_ang_line.get_x(c[3].y), c[3].y};
                    break;
                case 2:
                    u2 = Point{c[2].x, max_ang_line.get_y(c[2].x)};
                    break;
                case 3:
                    u2 = Point{max_ang_line.get_x(c[1].y), c[1].y};
                    break;
                }

                double max_u = std::max(l.calculate_distance(u1),
                                        l.calculate_distance(u2));
                double min_u = std::min(l.calculate_distance(u1),
                                        l.calculate_distance(u2));

                double max_l = std::max(l.calculate_distance(l1),
                                        l.calculate_distance(l2));
                double min_l = std::min(l.calculate_distance(l1),
                                        l.calculate_distance(l2));
