...
- The stab loop lives once in `simplify_core.h`; `simplify` and `simplify_with_gui` both call it (the GUI draws through an observer). The `--web-server` trace in `simplify.cpp` still keeps its own pre-optimization copy.
- Headers shared with `simplify_with_gui` compile under Qt, where `emit` is a macro; avoid using it as an identifier even when headless CI accepts it.
