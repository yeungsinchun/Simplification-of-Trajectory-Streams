...
**Fix:** Spotlight pad shrinks symmetrically near viewport edges (accounting for the 2px border) and measures room to the edge so header controls keep visible top/bottom margin inside the circle. Visibility uses laid-out size, not the class name. Backdrop uses `pointer-events: none` while `.has-spotlight`. Start tour copy is ≤2 short sentences per step (ε/δ only on step 3; Choose step is preloaded-only, no desktop Upload mention). Tour has Back / ← to go to the previous step. Standing form labels and param chips show ε / δ (not Match / Grid). Drop-hint prose, format line, and desktop Instructions list removed; mobile start help is tips-only.
