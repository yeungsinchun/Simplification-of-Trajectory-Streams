...
- Opt-in phase timers: `./build/simplify <id> --time` writes a human TIMING SUMMARY plus machine `TIMER_MS <name> <ms> <calls>` lines on stderr (`timer.h`, with core phase sites in `simplify_core.h`).
