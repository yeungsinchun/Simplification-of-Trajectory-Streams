...
def gating_summary(docs: List[Dict[str, Any]]) -> str:
    counts = [d.get("gated_count", 0) for d in docs]
    return f"{sum(counts)}/{10 * len(docs)} IDs gated"

def phase_ms(value: Optional[float]) -> str:
    return f"{value:.2f}" if value is not None else "n/a"

def phase_share(value: Optional[float], total: Optional[float]) -> str:
    return f"{value / total * 100:.1f}%" if value is not None and total else "n/a"

def phase_speedup(orig: Optional[float], new: Optional[float]) -> str:
    if orig is None or new is None:
        return "n/a"
    if new == 0:
        return "∞" if orig else "n/a"
    return f"{orig / new:.2f}×"
...
def compute_phase_totals(docs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    all_phases_set = set()
    global_orig_available = True
    global_orig_phases = None
    global_new_phases = None
    ...
    for doc in docs:
        ...
        orig_available = bool(cases) and all(isinstance(c.get("orig_ops"), dict) and c["orig_ops"] for c in cases)
        global_orig_available &= orig_available
        orig_phases = None
        new_phases = None
        ...
        for case in cases:
            ...
            orig_keys = set(orig_ops) if isinstance(orig_ops, dict) else set()
            new_keys = set(new_ops) if isinstance(new_ops, dict) else set()
            orig_phases = orig_keys if orig_phases is None else orig_phases & orig_keys
            new_phases = new_keys if new_phases is None else new_phases & new_keys
        ...
        for k, v in new_totals.items():
            global_new[k] += v
        global_orig_phases = (orig_phases or set()) if global_orig_phases is None else global_orig_phases & (orig_phases or set())
        global_new_phases = (new_phases or set()) if global_new_phases is None else global_new_phases & (new_phases or set())

        per_label.append({
            "label": label,
            "doc": doc,
            "orig_totals": dict(orig_totals),
            "orig_available": orig_available,
            "orig_phases": orig_phases or set(),
            "new_totals": dict(new_totals),
            "new_phases": new_phases or set(),
    ...
    return {
        "per_label": per_label,
        "global_orig": dict(global_orig) if global_orig_available else {},
        "global_orig_available": global_orig_available,
        "global_orig_phases": global_orig_phases or set(),
        "global_new": dict(global_new),
        "global_new_phases": global_new_phases or set(),
        "all_phases": all_phases,
        "leaf_phases": leaf_phases,
    }

def render_markdown(docs: List[Dict[str, Any]], phase_info: Dict[str, Any], run_url: str, run_id: str, expected_configs: Optional[int] = None) -> str:
    lines: List[str] = []
    ...
    lines.append("")
    all_ratios = [d.get("mean_ratio") for d in docs if isinstance(d.get("mean_ratio"), (int,float)) and not math.isnan(d.get("mean_ratio", float('nan')))]
    if all_ratios:
        ...
        lines.append(f"**Headline:** {len(docs)} configurations ({gating_summary(docs)}).")
        lines.append(f"Speedups (orig/new): **{min_sp:.2f}–{max_sp:.2f}×** (mean {mean_sp:.2f}×) on mean (new/orig ratios {min(all_ratios):.3f}–{max(all_ratios):.3f}).")
        lines.append("")
        # Also show pass/fail counts
        pass_cnt = sum(1 for d in docs if d.get("overall_ok"))
        lines.append(f"Gating: {pass_cnt}/{expected_configs or len(docs)} configurations passed (mean ≤1.05× and per-ID ≤1.20×).")
        lines.append("")
    else:
        lines.append(f"{len(docs)} configurations.")
        lines.append("")

    if expected_configs is not None and len({d.get("label") for d in docs}) != expected_configs:
        lines.append(f"**Incomplete matrix:** {len({d.get('label') for d in docs})}/{expected_configs} configurations reported.")
        lines.append("")

    # Overall per-configuration table
    lines.append("## Overall per configuration")
    lines.append("")
    lines.append("| Configuration | ε | δ | size | orig ms (mean) | new ms (mean) | speedup (orig/new) | gated IDs | status |")
    lines.append("|---|---|---|---|---|---|---|---|---|")
    for doc in docs:
        ...
        # Format ms
        orig_s = fmt_ms(mean_orig)
        new_s = fmt_ms(mean_new)
        lines.append(f"| {label} | {eps} | {delta:.6g} | {size} | {orig_s} | {new_s} | {speed_s} | {gated}/10 | {status} |")
    lines.append("")
    lines.append(f"_Thresholds: per-ID new ≤ orig×1.20, mean(new) ≤ mean(orig)×1.05 ({gating_summary(docs)})._")
    lines.append("")
    ...
    orig_available = phase_info["global_orig_available"]
    ...
    for phase in leaf_phases:
        o_ms = global_orig.get(phase, 0) if orig_available and phase in phase_info["global_orig_phases"] else None
        ...
        if has_phase_data:
            ...
            for pl in phase_info["per_label"]:
                ...
                for ph in top_phases:
                    o = pl["orig_totals"].get(ph, 0) if pl["orig_available"] and ph in pl["orig_phases"] else None
                    n = pl["new_totals"].get(ph, 0) if ph in pl["new_phases"] else None
                    row += f" {phase_speedup(o, n)} |"
                lines.append(row)
            lines.append("")
    ...
    lines.append("---")
    lines.append(f"_Generated by `scripts/ci/bench_report.py` from `benchmark.json` artifacts. {gating_summary(docs)}._")
    if run_url:
        lines.append(f"_Run: {run_url}_")
    lines.append("")
    return "\n".join(lines)

def render_html(docs: List[Dict[str, Any]], phase_info: Dict[str, Any], run_url: str, run_id: str, sha: str = "", expected_configs: Optional[int] = None) -> str:
    all_ratios = [d.get("mean_ratio") for d in docs if isinstance(d.get("mean_ratio"), (int,float))]
    # speedups
    speedups = []
    for r in all_ratios:
        if r and r != 0:
...
<p class="muted">""")
    ...
    if docs:
        html_parts.append(f' · {len(docs)}/{expected_configs or len(docs)} configurations · {gating_summary(docs)}')
    if expected_configs is not None and len({d.get("label") for d in docs}) != expected_configs:
        html_parts.append(f' · INCOMPLETE MATRIX ({len({d.get("label") for d in docs})}/{expected_configs} reported)')
    html_parts.append("</p>")

    # Headline card
    html_parts.append('<div class="card">')
    if speedups:
        html_parts.append(f'<div class="headline">{headline_speedup_range} <small>speedup (orig/new) · mean {mean_sp:.2f}× · {pass_cnt}/{expected_configs or len(docs)} passed</small></div>')
        html_parts.append(f'<p class="muted">Mean ratios (new/orig): {min(all_ratios):.3f}–{max(all_ratios):.3f}. Per-ID limit 1.20×, mean limit 1.05×.</p>')
    else:
  ...
  <span><i style="background:#b692ff"></i> Gi prep</span>
    ...
    html_parts.append('<thead><tr><th>Configuration</th><th>ε</th><th>δ</th><th>orig ms<br><span class="muted">mean</span></th><th>new ms<br><span class="muted">mean</span></th><th>speedup<br><span class="muted">orig/new</span></th><th>gated</th><th>status</th><th>time bar<br><span class="muted">orig → new</span></th></tr></thead><tbody>')
    for doc in docs:
        label = doc.get("label","")
        eps = doc.get("epsilon","")
        delta = doc.get("delta","")
    ...
    html_parts.append(f'<p class="muted">Thresholds: per-ID new ≤ orig×1.20, mean(new) ≤ mean(orig)×1.05 ({gating_summary(docs)}).</p></div>')
    ...
    for phase in leaf_phases:
        ...
        o_w = (o_ms or 0) / max_phase * 80
        n_w = (n_ms or 0) / max_phase * 80
    ...
    if per_label and any(pl["orig_totals"] or pl["new_totals"] for pl in per_label):
        ...
        for pl in per_label:
            ...
            for ph in phases_here:
                o_ms = orig_totals.get(ph, 0) if pl["orig_available"] and ph in pl["orig_phases"] else None
                n_ms = new_totals.get(ph, 0) if ph in pl["new_phases"] else None
                col = phase_color(ph)
                o_w = (o_ms or 0) / max_here * 70 if max_here else 0
                n_w = (n_ms or 0) / max_here * 70 if max_here else 0
                ...
                html_parts.append(f'<tr><td class="mono">{ph}</td><td>{phase_ms(o_ms)}</td><td>{phase_share(o_ms, orig_total)}</td><td>{phase_ms(n_ms)}</td><td>{phase_share(n_ms, new_total)}</td><td>{phase_speedup(o_ms, n_ms)}</td><td>{bar}</td></tr>')
            # Totals row
            html_parts.append(f'<tr style="font-weight:700"><td>total</td><td>{phase_ms(orig_total if pl["orig_available"] else None)}</td><td>{phase_share(orig_total if pl["orig_available"] else None, orig_total)}</td><td>{phase_ms(new_total)}</td><td>100%</td><td>{phase_speedup(orig_total if pl["orig_available"] else None, new_total)}</td><td></td></tr>')
            html_parts.append('</tbody></table></div>')
        html_parts.append('</div>')

    # Detailed per-ID table collapsible? Could add but keep concise
    html_parts.append(f'<div class="card"><h2>Per-ID details</h2><p class="muted">{gating_summary(docs)}. Ratio = new/orig.</p>')
    for doc in docs:
        label = doc.get("label","")
        cases = doc.get("cases", [])
        if not cases:
...
Generated by <span class="mono">scripts/ci/bench_report.py</span> from benchmark artifacts. Thresholds: per-ID new ≤ orig×1.20, mean(new) ≤ mean(orig)×1.05 ({gating_summary(docs)}). TIMER_MS phase times are from one <span class="mono">--time</span> run per ID and summed per configuration.
</div>""")
    html_parts.append("</div></body></html>")
    return "\n".join(html_parts)

def render_comment(docs: List[Dict[str, Any]], phase_info: Dict[str, Any], run_url: str, expected_configs: Optional[int] = None) -> str:
    lines: List[str] = []
    lines.append("<!-- bench-report -->")
    lines.append("## Benchmark Report")
    if run_url:
        lines.append(f"Run: {run_url}")
    ratios = [d.get("mean_ratio") for d in docs if isinstance(d.get("mean_ratio"), (int,float))]
    if ratios:
        speedups = [1/r for r in ratios if r]
        lines.append(f"**Speedup (orig/new) on means: {min(speedups):.2f}–{max(speedups):.2f}× (mean {sum(speedups)/len(speedups):.2f}×)** — {len(docs)}/{expected_configs or len(docs)} configurations ({gating_summary(docs)}).")
    # Pass/fail
    pass_cnt = sum(1 for d in docs if d.get("overall_ok"))
    lines.append(f"Gating: **{pass_cnt}/{expected_configs or len(docs)} passed** (per-ID ≤1.20×, mean ≤1.05×).")
    if expected_configs is not None and len({d.get("label") for d in docs}) != expected_configs:
        lines.append(f"**Incomplete matrix:** {len({d.get('label') for d in docs})}/{expected_configs} configurations reported.")
    lines.append("")
    ...
    if leaf_phases and global_new:
        ...
        lines.append("")
        for ph in leaf_phases:
            o = global_orig.get(ph,0) if phase_info["global_orig_available"] and ph in phase_info["global_orig_phases"] else None
            n = global_new.get(ph,0) if ph in phase_info["global_new_phases"] else None
            lines.append(f"- {ph}: {phase_ms(o)} → {phase_ms(n)} ms ({phase_speedup(o, n)}, {phase_share(o, total_o)} → {phase_share(n, total_n)} share)")
        lines.append("")
...
def main() -> int:
    ...
    if not paths:
        ...
        phase_info = {"per_label": [], "global_orig": {}, "global_new": {}, "global_orig_available": False, "global_orig_phases": set(), "global_new_phases": set(), "all_phases": [], "leaf_phases": []}
    else:
        docs = []
        for p in paths:
            d = load_report(p)
    ...
    md = render_markdown(docs, phase_info, run_url, args.run_id, args.expected_configs)
    html = render_html(docs, phase_info, run_url, args.run_id, args.sha, args.expected_configs)
    comment = render_comment(docs, phase_info, run_url, args.expected_configs)
