...
class OPERB : public Algorithm {
  ...
  public:
    ...
    Trajectory<Line> *compress(const Trajectory<Point> *traj) {
        ...
        if (traj->size() == 2) {
            ...
            while (next_point.first != -1) {
                ...
                next_point =
                    get_active_point(traj, s_index, next_point.first, fit_line);
                ...
                while (next_point.first != -1 && next_point.second == true) {
                    ...
                    next_point = get_active_point(traj, s_index,
                                                  next_point.first, fit_line);
                }
    ...
    std::pair<int, bool> get_active_point(const Trajectory<Point> *traj,
                                          int s_index, int a_index, FitLine L) {

        int i = a_index + 1;
        bool flag = true;
        Line R{(*traj)[s_index], (*traj)[i]};

        while (i - s_index <= magic_number && i < traj->size() &&
               R.length() - L.length() < bound / 4) {

            if (L.calculate_distance((*traj)[i]) > bound / 2 &&
                R.calculate_distance((*traj)[i]) > bound) {
                flag = false;
                break;
            }
            i++;
...
class OPERBA : public Algorithm {
  ...
  public:
    OPERBA(double bound_, double gamma_y_ = acos(-1) / 3.0)
        : Algorithm{bound_}, gamma_y{gamma_y_} {}
    ...
    Trajectory<Line> *compress(const Trajectory<Point> *traj) {
        ...
        if (traj->size() == 2) {
            ...
            int s_index = 0, e_index = 0;
            Line R_pre, R_ano;
            bool R_pre_null = true,
                 R_ano_null = true; // to identify whether they are null or not.
            ...
            while (next_point.first != -1) {
                ...
                if (next_point.first != -1 && R_ano_null == false &&
                    ...
                    if (R_pre_null != true) {
                        auto patch_result =
                            patch(R_pre, R_ano,
                                  Line{(*traj)[s_index], (*traj)[e_index]});
    ...
    std::pair<std::pair<Line, Line>, bool> patch(Line R_pre, Line R_ano,
                                                 Line R_suc) {
        ...
        if (R_pre.length() - L_a.length() < bound / 2 &&
            (-acos(-1) * 2.0 < angle_difference &&
                 angle_difference <= -acos(-1) - gamma_y ||
             gamma_y - acos(-1) <= angle_difference &&
                 angle_difference <= acos(-1) - gamma_y ||
             acos(-1) + gamma_y <= angle_difference &&
                 angle_difference < 2 * acos(-1))) {
            return {{L_a, L_b}, true};
        } else
